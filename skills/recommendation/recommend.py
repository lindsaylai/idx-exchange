...
def _embed_batch(target_text: str, candidate_texts: list[str]):
    ...
    land in the same vector space. Tries the local embedding model first;
    on any failure (rare -- missing cached weights, no network on a
    first-ever run) falls back to a TF-IDF vectorizer fit fresh on this
    batch -- same graceful-degradation pattern as
