...
- **Embeds fresh per call, not from the Week 6 cache:** the candidate set
  ...
  cost.
- **Same TF-IDF fallback as Week 6:** if the local embedding model fails
  (rare — no cached weights and no network on a first-ever run), falls
  back to a `TfidfVectorizer` fit fresh on the target + this call's
  candidate remarks.
